...
$config_test=<<SCRIPTTEST
  ...
  echo "traceroute -I anode \>\> \/tmp\/test\.out" >> /tmp/test.sh
  echo "ping \-c 2 bnode \>\> \/tmp\/test\.out" >> /tmp/test.sh
  echo "traceroute -I bnode \>\> \/tmp\/test\.out" >> /tmp/test.sh
  echo "ping \-c 2 cnode \>\> \/tmp\/test\.out" >> /tmp/test.sh
  echo "traceroute -I cnode \>\> \/tmp\/test\.out" >> /tmp/test.sh
  echo "ping \-c 2 dnode \>\> \/tmp\/test\.out" >> /tmp/test.sh
  echo "traceroute -I dnode \>\> \/tmp\/test\.out" >> /tmp/test.sh
  echo "ping \-c 2 enode \>\> \/tmp\/test\.out" >> /tmp/test.sh
  echo "traceroute -I enode \>\> \/tmp\/test\.out" >> /tmp/test.sh
  echo "echo INTERNET TEST \>\> \/tmp\/test\.out" >> /tmp/test.sh
  echo "ping \-c 2 google.com 2>&1 \>\> \/tmp\/test\.out" >> /tmp/test.sh
  echo "traceroute -I google.com 2>&1 \>\> \/tmp\/test\.out" >> /tmp/test.sh
...
######
  ...
  config.vm.define "bnode" do |bnode|
    ...
    bnode.vm.provision "shell", inline: $config_test
  end
  ...
  config.vm.define "cnode" do |cnode|
    ...
    cnode.vm.provision "shell", inline: $config_test
  end
  ...
  config.vm.define "dnode" do |dnode|
    ...
    dnode.vm.provision "shell", inline: $config_test
  end
  ...
  config.vm.define "enode", primary: true do |enode|
    ...
    enode.vm.provision "shell", inline: $config_test
  end
